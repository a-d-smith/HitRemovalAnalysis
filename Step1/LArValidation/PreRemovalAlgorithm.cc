...
PreRemovalAlgorithm::PreRemovalAlgorithm(){
    ...
    m_outputPfoPrimaryTreeName = "HitDataPfoPrimary"; 
...
PreRemovalAlgorithm::~PreRemovalAlgorithm(){
    ...
    PANDORA_MONITORING_API(SaveTree(this->GetPandora(), m_outputPfoPrimaryTreeName.c_str(), m_outputFileName.c_str(), "UPDATE"));
...
StatusCode PreRemovalAlgorithm::Run()
{
    ...
    // Get information on primary pfos (folding daughter particles in too!)
    // --------------------------------------------------------------------

    // A map between hit and Pfo uids
    std::map<size_t, size_t> hitPfoPrimaryMap;
    ...
    LArMonitoringHelper::GetPfoToCaloHitMatches(pCaloHitList, *pPfoList, true, hitToPfoMap, pfoToHitListMap);

    std::cout << "nPFOs        = " << pPfoList->size() << std::endl;
    std::cout << "nPrimaryPFOs = " << pfoToHitListMap.size() << std::endl;
    ...
    for (itPfo = uidPfoMap.begin(); itPfo != uidPfoMap.end(); ++itPfo){
        ...
        LArMonitoringHelper::PfoContributionMap::iterator itPfoPrimary;
        for (itPfoPrimary = pfoToHitListMap.begin(); itPfoPrimary != pfoToHitListMap.end(); ++itPfoPrimary) {
            if (itPfo->second == itPfoPrimary->first){
                // Pfo is primary
                // -------------------------------------------------------
                // Get a list of all of the hit uids
                std::vector<int> *pfoHitList = new std::vector<int>;
                for (const CaloHit *const &pCaloHit : itPfoPrimary->second){
                    // Find the id of this calohit
                    size_t hitUid;
                    bool foundHitId = false;
                    std::map<size_t, const CaloHit *const>::iterator it;
                    ...
                    pfoHitList->push_back((int) hitUid);
                    hitPfoPrimaryMap.insert(std::pair<size_t, size_t>(hitUid, uid));
                }
                // Output this information to a tree
                // --------------------------------------------------
                // Identifier
                PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_outputPfoPrimaryTreeName.c_str(), "FileId"    , m_fileId));
                PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_outputPfoPrimaryTreeName.c_str(), "EventId"   , m_eventNumber));
                PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_outputPfoPrimaryTreeName.c_str(), "UniqueId"  , (int) uid));
                // Associated Hit List
                PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_outputPfoPrimaryTreeName.c_str(), "HitUidList", pfoHitList));
        
                PANDORA_MONITORING_API(FillTree(this->GetPandora(), m_outputPfoPrimaryTreeName.c_str()));
    ...
    m_eventNumber++;
    std::cout << "I have finished running" << std::endl;
...
StatusCode PreRemovalAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
    ...
    //m_outputFileName = "test.root";
    std::cout << "I have done my read settings" << std::endl;
